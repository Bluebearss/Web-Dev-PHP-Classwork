...
<html>
	<head>
		...
		<script type = "text/javascript">
			...
			<?php
				$conn = new PDO("mysql:hostname=localhost;dbname=brian_zhu", "root", "");
				srand();
				...
				if (isset($_POST['actiongrp']))
				{
					if ($_POST['actiongrp'] == "take")
					{
						...
						{
							$cmd = "INSERT INTO `items table` VALUES('$currentRoom', '$currentItem')";
							getDatabaseResults($cmd);
						}
						
						$currentItem = $_POST['items'];

						$cmd = "DELETE FROM `items table` WHERE `item` = '$currentItem'";
						getDatabaseResults($cmd);
					}
				...
				function getRandomRoom()
				{
					...
					$cmd = "SELECT DISTINCT `room` FROM `room table`";
				...
				{
					$cmd = "SELECT `connections` FROM `room table` WHERE `room` = '$currRoom'";
					$data = getDatabaseResults($cmd);
				...
				{
					$cmd = "SELECT `item` FROM `items table` WHERE `room` = '$currRoom'";
					$data = getDatabaseResults($cmd);
